...
def convert_salary(salary):
    ...
    if '元/月' in salary:
        ...
        try:
            min_salary, max_salary = map(lambda x: int(float(x.replace('万', ''))* 10)  if '万' in x else int(x), base_salary.split('-'))
            return f"{min_salary}k-{max_salary}k{multiplier}"
        except ValueError:
            if '万' in base_salary:
                if '千' in base_salary:
...
# 保存修改后的文件
df.to_csv('unified_智联.csv', index=False, encoding='utf-8')
