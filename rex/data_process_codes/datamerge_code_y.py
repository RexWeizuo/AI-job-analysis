...
# 检测文件编码
file_encoding = detect_encoding('A20231227.csv')
# 输出检测到的编码
print(f"Detected encoding: {file_encoding}")
# 读取CSV文件
df1 = pd.read_csv('A20231227.csv', encoding=file_encoding)
# # 读取第一个CSV文件
...
# 检测文件编码
file_encoding = detect_encoding('B20231227.csv')
# 输出检测到的编码
print(f"Detected encoding: {file_encoding}")
# 读取CSV文件
df2 = pd.read_csv('B20231227.csv', encoding=file_encoding)
# # 读取第二个CSV文件
...
# 加上更新时间列，统一填写20231224
merged_df['更新时间'] = '2023-12-27'

# 保存合并后的文件
merged_df.to_csv('猎聘_20231227.csv', index=False, encoding='utf-8')
